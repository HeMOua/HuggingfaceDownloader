...
class HuggingFaceDownloader(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("HuggingFace 模型下载器 v1.0")
...
class FileSelectionDialog(QMessageBox):
    ...
    def init_ui(self):
        ...
        self.checkboxes = []
        for file in self.files[:50]:  # 限制显示前50个文件
            checkbox = QCheckBox(file)
            self.checkboxes.append(checkbox)
            layout.addWidget(checkbox)
        
        if len(self.files) > 50:
            label = QLabel(f"... 还有 {len(self.files) - 50} 个文件未显示")
        ...
        self.layout().addWidget(scroll_area, 1, 0, 1, 2)
